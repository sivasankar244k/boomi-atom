...
wget $URL/atom/molecule_install64_no_jre.sh
chmod a+x molecule_install64_no_jre.sh
./molecule_install64_no_jre.sh -q -Vusername=${BOOMI_USERNAME} -Vpassword=${BOOMI_PASSWORD} -VatomName='${BOOMI_CONTAINERNAME}' \
  ...
  -dir ${INSTALLATION_DIRECTORY} -VinstallToken=${INSTALL_TOKEN} > ${ATOM_HOME}/install_Molecule_${DIRNAME}.log
